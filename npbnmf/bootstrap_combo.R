...
for (run in 1:datasets) {
  bs_list_lower[[run]] <- apply(bs_list[[run]], c(1,2), quantile, 0.025, na.rm = TRUE)
  bs_list_upper[[run]] <- apply(bs_list[[run]], c(1,2), quantile, 0.975, na.rm = TRUE)
  bs_list_mean[[run]]  <- apply(bs_list[[run]], c(1,2), mean, na.rm = TRUE)
}
