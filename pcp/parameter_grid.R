...
## Vary $\mu$ and $\lambda$
mu_values <- seq(sqrt(m/(2*log(n*m))), sqrt(n/2), by = 0.5)
lam_values <- seq(1/sqrt(n), 1/sqrt(m), 0.1)

seeds = 1:100

param_grid <- expand.grid(seeds = seeds, lam_values = lam_values, mu_values =  mu_values) %>% as_tibble()
...
save(all_out, file = paste0("/ifs/scratch/msph/ehs/eag2186/pcp/param_send/param_out_", 
     job_num, "_upper.RDA"))
