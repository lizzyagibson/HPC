...
## Vary $\mu$ and $\lambda$
mu_values <- seq(mu_mix, sqrt(nn/2), by = 0.5)
...
range_out1 <- param_grid[job_num, ] %>% 
  mutate(pcp_out = pmap(list(lam_values, mu_values),
                        pcp_func))

range_out <- range_out1  %>% 
...
save(all_out, file = paste0("/ifs/scratch/msph/ehs/eag2186/pcp/boston_param/boston_param_out_", 
     job_num, "_upper.RDA"))
